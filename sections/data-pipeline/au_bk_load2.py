...
# Name connection to MySQL in Airflow, REST API Conversion rate
...
def get_data_from_mysql(transaction_path):
    # Run MySqlHook to connect to MySQL from a connection created in Airflow    
    mysqlserver = MySqlHook(MYSQL_CONNECTION)
    # Query from database using a hook
...
def get_conversion_rate(conversion_rate_path):
    ...
    # Convert the date index to a column named "date" and save the CSV file.
...
def merge_data(transaction_path, conversion_rate_path, output_path):
    ...
    # Convert the price by removing the $ sign and convert it to a float.
...
# Instantiate a Dag
with DAG( 
    "audible_books_revenue",
    start_date=days_ago(1),
    schedule_interval="@once",
    tags=["project"]
) as dag:

    # Task1 get_data_from_mysql Function
    t1 = PythonOperator(
    ...
    # Task2 get_conversion_rate Function
    t2 = PythonOperator(
    ...
    # Task4 Import file output.csv to BigQuery
    t4 = GCSToBigQueryOperator(
        task_id='gcs_to_bq',
        bucket='asia-east2-audible-books-re-05867ec0-bucket',
        source_objects=['data/output.csv'],
        destination_project_dataset_table='project.audible_data',
        skip_leading_rows=1,
        schema_fields=[
            {
                "mode": "NULLABLE",
